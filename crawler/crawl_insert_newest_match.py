        ...
        with connection.cursor() as cursor:
            for i, match_id in enumerate(match_ids):
                ...
                if match_id in cur_match_ids:
                    ...
                    cursor.execute(update_match_had, (all_data[0][0], all_data[0][1], all_data[0][2], match_ids[i]))
                    
                    update_match_hhad = """ UPDATE match_hhad SET h = %s, a = %s, d = %s WHERE match_id = %s """
                    cursor.execute(update_match_hhad, (all_data[1][0], all_data[1][1], all_data[1][2], match_ids[i]))
                    
                    update_match_ttg = """ UPDATE match_ttg SET s0 = %s, s1 = %s, s2 = %s, s3 = %s, s4 = %s, s5 = %s, s6 = %s, s7=%s
                    WHERE match_id = %s
                    """
                    cursor.execute(update_match_ttg, (all_data[2][0], all_data[2][1], all_data[2][2], all_data[2][3], 
                                                      all_data[2][4], all_data[2][5], all_data[2][6], all_data[2][7], match_ids[i]))
                    ...
                    WHERE match_id=%s
                    """
                    lst = [all_data[3][i] for i in range(len(all_data[3]))]
                    lst.append(match_id)
                    ...
                    WHERE match_id=%s
                    """
                    lst = [all_data[4][i] for i in range(len(all_data[4]))]
                    lst.append(match_id)
